...
def test_end_to_end_cap_and_duplicate_collapse():
    ...
    with patch.dict(
        os.environ, {"AUTO_CREATE_TICKET": "true", "COMMENT_ON_DUPLICATE": "false"}
    ):
        with (
            patch("agent.config.get_config", return_value=cfg),
            ...
            with patch(
                "agent.nodes.analysis._call_llm_with_circuit_breaker", async_mock
            ):
                # Disable Jira search (simulate failure/skip) so local fingerprinting is decisive
                with (
                    patch("agent.jira.client.search", return_value=None),
                    patch("agent.jira.match.client.search", return_value=None),
                ):
                    with (
                        patch("agent.jira.client.is_configured", return_value=True),
                        ...
                        with patch("agent.jira.jira_create_issue") as mock_create:
                            mock_create.side_effect = lambda payload: {
                                "key": f"TEST-{mock_create.call_count}"
                            }
                            # Prevent fingerprint persistence during test
                            # Patch fingerprint functions at all import sites
                            with (
                                patch("agent.jira.utils.save_processed_fingerprints"),
                                patch(
                                    "agent.jira.utils.load_processed_fingerprints",
                                    return_value=set(),
                                ),
                                patch(
                                    "agent.jira.load_processed_fingerprints",
                                    return_value=set(),
                                ),
                                patch("agent.jira.save_processed_fingerprints"),
                                patch(
                                    "agent.nodes.ticket._load_processed_fingerprints",
                                    return_value=set(),
                                ),
                                patch(
                                    "agent.nodes.ticket._save_processed_fingerprints"
                                ),
