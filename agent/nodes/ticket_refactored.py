...
def _build_labels(state: Dict[str, Any], fingerprint: str) -> list[str]:
    ...
    if (
        os.getenv("AGGREGATE_EMAIL_NOT_FOUND", "false").lower() in ("1", "true", "yes")
        and etype == "email-not-found"
    ):
        labels.append("aggregate-email-not-found")

    if (
        os.getenv("AGGREGATE_KAFKA_CONSUMER", "false").lower() in ("1", "true", "yes")
        and etype == "kafka-consumer"
    ):
...
def _clean_title(title: str, error_type: Optional[str]) -> str:
    ...
    if error_type == "email-not-found" and os.getenv(
        "AGGREGATE_EMAIL_NOT_FOUND", "false"
    ).lower() in ("1", "true", "yes"):
        base_title = "Investigate Email Not Found errors (aggregated)"
    elif error_type == "kafka-consumer" and os.getenv(
